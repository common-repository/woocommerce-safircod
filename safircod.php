    ...
    public function save_order($id, $posted)
    {
        ...
        $params = array(
         ...
         if ($res === false) {
            ...
            $this->trigger($order->id, $order, true, $response->RegisterNewOrderResult);
            ...
            $html .= '</br>';
            $html .= 'این کد را نزد خود نگه‌دارید و با مراجعه به سایت پست از وضعیت سفارش خود آگاه شوید. ';
            $html .= '</br>'. $response->RegisterNewOrderResult .'</p><div class="clear"></div>';
            wc_add_notice($html);
         } else {
        ...
        if(!$subject) {
		  ...
		  $this->email_handle->send( get_option( 'admin_email' ), sprintf('سفارش  %s در سیستم سفیر ثبت نشد', $order->get_order_number() ), $message );
            ...
            $message = $this->email_handle->wrap_message(
		            		...
		            		sprintf( 'سفارش  %s با موفقیت در سیستم سفیر ثبت گردید. شماره رهگیری پستی: %s', $order->get_order_number() ,$factor)
						);
